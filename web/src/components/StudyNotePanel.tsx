    ...
    <aside className="fixed top-4 bottom-20 z-50" style={{ [sidePos]: 12, width, height, minWidth: 360, minHeight: 400 }}>
      <div className="relative h-full w-full rounded-2xl bg-[#8c582b] p-3 shadow-2xl">
        <div className="h-full w-full overflow-hidden rounded-xl border-4 border-[#70431f] bg-[#175b37]">
          ...
          {toolbarVisible && <div className="mx-2 mt-2 flex min-h-11 items-center gap-1 rounded-md bg-[#28313c] px-2 py-1 text-white">
          ...
          {!toolbarVisible && <button className="m-2 text-xs text-white/70" onClick={() => setToolbarVisible(true)}>展开工具栏</button>}
        ...
        <div className="absolute -right-1 top-1/2 h-16 w-3 cursor-ew-resize rounded bg-[#f0a060]" title="拖拽调节区域大小" onMouseDown={(e) => { const start = e.clientX; const initial = width; const move = (event: MouseEvent) => setWidth(Math.max(360, Math.min(1000, initial + (sidePos === "right" ? start - event.clientX : event.clientX - start)))); const up = () => { window.removeEventListener("mousemove", move); window.removeEventListener("mouseup", up); }; window.addEventListener("mousemove", move); window.addEventListener("mouseup", up); }} />
      </div>
    </aside>
  );
}

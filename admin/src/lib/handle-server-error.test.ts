...
describe('handleServerError', () => {
  ...
  it('prefers CloudSteps msg over title on Axios errors', () => {
    const error = new AxiosError('Bad request')
    error.response = {
      status: 400,
      data: { msg: '验证码错误', title: 'Validation failed' },
    } as AxiosError['response']

    handleServerError(error)

    expect(toastError).toHaveBeenCalledWith('验证码错误')
  })

  it('reads msg from a CloudSteps unwrap error', () => {
    handleServerError(Object.assign(new Error('请求失败'), { msg: '权限不足' }))

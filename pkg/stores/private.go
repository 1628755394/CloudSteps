...
// callers should fall back to the server-side multipart upload endpoints.
var ErrDirectUploadUnsupported = &utils.Error{Code: http.StatusNotImplemented, Message: "direct upload not supported by this storage backend"}

// PrivateURLSigner is implemented by stores that can mint expiring signed GET
// URLs for objects in a private bucket (私有空间私有访问).
...
// upload (browser/device → storage, 客户端直传) without proxying the object

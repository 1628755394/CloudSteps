...
// SignedURL implements PrivateURLSigner via a presigned GET (OBS is
// S3-compatible, so the aws-sdk presigner applies).
func (o *ObsStore) SignedURL(key string, expires time.Duration) (string, error) {
	ctx := context.Background()
	client, err := o.client(ctx)
	if err != nil {
		return "", err
...
// PresignUpload implements DirectUploadPresigner via a presigned PUT.
func (o *ObsStore) PresignUpload(key, contentType string, expires time.Duration) (*DirectUpload, error) {
	ctx := context.Background()
	client, err := o.client(ctx)
	if err != nil {
		return nil, err

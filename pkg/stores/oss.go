...
func (o *OSSStore) bucket() (*oss.Bucket, error) {
	...
	return client.Bucket(o.BucketName)
}

// SignedURL implements PrivateURLSigner via an OSS signed GET URL.
func (o *OSSStore) SignedURL(key string, expires time.Duration) (string, error) {
	bucket, err := o.bucket()
	if err != nil {
		return "", err
	}
...
// PresignUpload implements DirectUploadPresigner via an OSS signed PUT URL.
func (o *OSSStore) PresignUpload(key, contentType string, expires time.Duration) (*DirectUpload, error) {
	bucket, err := o.bucket()
	if err != nil {
		return nil, err
	}

...
// SignedURL implements PrivateURLSigner via an S3 presigned GET.
func (s *S3Store) SignedURL(key string, expires time.Duration) (string, error) {
	ctx := context.Background()
	client, err := s.client(ctx)
	if err != nil {
		return "", err
...
// PresignUpload implements DirectUploadPresigner via an S3 presigned PUT.
func (s *S3Store) PresignUpload(key, contentType string, expires time.Duration) (*DirectUpload, error) {
	ctx := context.Background()
	client, err := s.client(ctx)
	if err != nil {
		return nil, err

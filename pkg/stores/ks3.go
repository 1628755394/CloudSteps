...
// SignedURL implements PrivateURLSigner via a KS3 presigned GET.
func (s *Ks3Store) SignedURL(key string, expires time.Duration) (string, error) {
	client := s.client()
...
// PresignUpload implements DirectUploadPresigner via a KS3 presigned PUT.
func (s *Ks3Store) PresignUpload(key, contentType string, expires time.Duration) (*DirectUpload, error) {
	client := s.client()

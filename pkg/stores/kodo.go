...
func (q *QiNiuStore) normalizedDomain() string {
	d := q.Domain
...
// SignedURL implements PrivateURLSigner: 私有空间按调用方指定的有效期签发下载 URL。
func (q *QiNiuStore) SignedURL(key string, expires time.Duration) (string, error) {
	if q.Domain == "" {
		return "", ErrInvalidPath
	}
	deadline := time.Now().Add(expires).Unix()
	return storage.MakePrivateURL(q.getMac(), q.normalizedDomain(), key, deadline), nil
}

// PresignUpload implements DirectUploadPresigner: 客户端直传凭证（表单上传）。
// 浏览器向返回的 URL 发 multipart/form-data，携带 Form 字段和 FileField 文件。
// Scope 绑定到具体 key，凭证只能覆盖这一个对象。
func (q *QiNiuStore) PresignUpload(key, contentType string, expires time.Duration) (*DirectUpload, error) {
	policy := storage.PutPolicy{
		Scope:   q.BucketName + ":" + key,
		Expires: uint64(expires / time.Second),
	}
	if contentType != "" {
		policy.MimeLimit = contentType
	}
	token := policy.UploadToken(q.getMac())

	upHost := "https://upload.qiniup.com"
	if zone, err := storage.GetRegion(q.AccessKey, q.BucketName); err == nil && zone != nil && len(zone.SrcUpHosts) > 0 {
		h := zone.SrcUpHosts[0]

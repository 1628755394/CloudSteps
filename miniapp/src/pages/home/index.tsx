/**
 * 首页(tab) — 对齐 web/src/pages/Home.tsx + CoachingSchedulePanel.tsx。
 *
...
export default function Home() {
  ...
  const renderScheduleCard = (s: CoachingWeekSchedule) => {
    ...
    return (
      <View
        key={s.id}
        className={`home__schedule-card ${canEnter ? 'home__schedule-card--clickable' : ''}`}
        onClick={() => canEnter && Taro.navigateTo({ url: '/pages/material-selection/index' })}
      >
        <View className="home__schedule-main">
          <Text className="home__schedule-title">{s.title || `排课 #${s.id}`}</Text>
          <View className="home__schedule-meta">
            <View className="home__schedule-meta-item">
              <Clock size={14} color="#787671" />
              <Text className="home__schedule-meta-text">
                {s.scheduledDate?.slice?.(0, 10) || s.scheduledDate} · {s.startTime}–{s.endTime}
              </Text>
            </View>
            {s.students && s.students.length > 0 && (
              <View className="home__schedule-meta-item">
                <Text className="home__schedule-meta-text">{s.students.join('、')}</Text>
              </View>
            )}
          </View>
          {st === 'scheduled' && !isPastSlot && (
            <Text className="home__schedule-hint home__schedule-hint--mint">可提前开始上课</Text>
          )}
          {isPastSlot && (
            <Text className="home__schedule-hint home__schedule-hint--muted">计划时段已过，仍可开始或删除</Text>
          )}
          {st === 'in_progress' && minsLeft != null && (
            <Text className="home__schedule-hint home__schedule-hint--mint">
              上课中 · 距结束约 {minsLeft} 分钟
            </Text>
          )}
        </View>
        <View className="home__schedule-actions">
          {st === 'scheduled' && (
            <View
              className="home__schedule-btn home__schedule-btn--danger"
              onClick={(e) => { e.stopPropagation(); onDelete(s.id) }}
            >
              <Text className="home__schedule-btn-text home__schedule-btn-text--danger">删除</Text>
            </View>
          )}
          {canStart && (
            <CloudButton
    ...
    <ScrollView className="home" scrollY enableFlex>
      ...
      <View className="home__hero">
        <View className="home__hero-info">
          <Text className="home__greeting">{isCoach ? '陪练排课' : '我的课表'}</Text>
          <Text className="home__week-range">{weekRangeLabel}</Text>
        </View>
        <View
      ...
      {/* 周切换 */}
      <View className="home__week-nav">
        <View className="home__week-nav-btn" onClick={() => setWeekAnchor(addDays(weekAnchor, -7))}>
          <Text className="home__week-nav-text">上一周</Text>
        </View>
        <View className="home__week-nav-btn" onClick={() => setWeekAnchor(new Date())}>
          <Text className="home__week-nav-text home__week-nav-text--primary">本周</Text>
        </View>
        <View className="home__week-nav-btn" onClick={() => setWeekAnchor(addDays(weekAnchor, 7))}>
          <Text className="home__week-nav-text">下一周</Text>
        </View>
      </View>

      {/* 教练: 下一节课 + 统计 */}
      {isCoach && (
        <View className="home__coach-grid">
          <View
            className={`home__next-card ${!nextClass ? 'home__next-card--disabled' : ''}`}
            onClick={() => {
              if (!nextClass) return
          ...
          >
            {nextClass ? (
              <>
                <Text className="home__next-title">
                  {nextClass.title || nextClass.students?.[0] || '当前课程'}
                </Text>
                <Text className="home__next-meta">
                  {nextClass.scheduledDate?.slice(0, 10)} · {nextClass.startTime}–{nextClass.endTime}
                </Text>
              </>
            ) : (
              <Text className="home__next-title home__next-title--muted">暂无待上课程</Text>
            )}
          </View>
          <View className="home__stat-card">
            <View className="home__stat-header">
              <Clock size={16} color="#55A3FF" />
              <Text className="home__stat-label">待上 / 进行中</Text>
            </View>
            <Text className="home__stat-value">{activeSchedules.length}</Text>
          </View>
        </View>
      )}
      ...
      {/* 教练: 新建排课 / 添加学员 */}
      {isCoach && (
        <View className="home__action-grid">
          <View
            className="home__action-btn home__action-btn--brand"
            onClick={() => { setShowStudentForm(false); setShowScheduleForm((v) => !v) }}
          >
            <Plus size={16} color="#fff" />
            <Text className="home__action-btn-text home__action-btn-text--white">新建排课</Text>
          </View>
          <View
            className="home__action-btn home__action-btn--outline"
            onClick={() => { setShowScheduleForm(false); setShowStudentForm((v) => !v) }}
          >
            <Plus size={16} color="#4ECDC4" />
            <Text className="home__action-btn-text">添加学员</Text>
          </View>
        </View>
      )}
      ...
      {isCoach && showScheduleForm && (
        <View className="home__form-card">
          <Text className="home__form-title">新建排课</Text>
          <View className="home__form-row">
            <Text className="home__form-label">学员</Text>
            <Picker
              mode="selector"
            ...
            >
              <View className="home__form-picker">
                <Text className={aStudent ? 'home__form-picker-text' : 'home__form-picker-text home__form-picker-text--placeholder'}>
                  {aStudent
          ...
          <View className="home__form-row">
            <Text className="home__form-label">日期</Text>
            <Picker
              mode="date"
              value={aDate}
            ...
            >
              <View className="home__form-picker">
                <Text className="home__form-picker-text">{aDate}</Text>
                <Right size={14} color="#a4a097" />
              </View>
            </Picker>
          </View>
          <View className="home__form-row home__form-row--two">
            <View className="home__form-col">
              <Text className="home__form-label">开始</Text>
              <Picker
                mode="time"
                value={aStart}
              ...
              >
                <View className="home__form-picker">
                  <Text className="home__form-picker-text">{aStart}</Text>
                  <Right size={14} color="#a4a097" />
                </View>
              </Picker>
            </View>
            <View className="home__form-col">
              <Text className="home__form-label">结束</Text>
              <Picker
                mode="time"
                value={aEnd}
              ...
              >
                <View className="home__form-picker">
                  <Text className="home__form-picker-text">{aEnd}</Text>
                  <Right size={14} color="#a4a097" />
                </View>
              </Picker>
            </View>
          ...
          <View className="home__form-row">
            <Text className="home__form-label">标题(可选)</Text>
            <Input
      ...
      {isCoach && showStudentForm && (
        <View className="home__form-card">
          <Text className="home__form-title">添加学员</Text>
          <Input
            className="home__form-input"
            value={searchQ}
          ...
          {searchResults.length > 0 && (
            <View className="home__search-results">
              {searchResults.map((u) => (
                <View
                  key={u.id}
                  className={`home__search-item ${pickedStudent?.id === u.id ? 'home__search-item--active' : ''}`}
                  onClick={() => setPickedStudent(u)}
                >
                  <Text className="home__search-name">{u.displayName || u.username}</Text>
                  <Text className="home__search-sub">
          ...
          <View className="home__form-row">
            <Text className="home__form-label">陪练剩余分钟</Text>
            <Input
              className="home__form-input"
              type="number"
      ...
      {/* 课表列表 */}
      <View className="home__schedule-list">
        {loading ? (
          <View className="home__state">
            <Text className="home__state-text">加载中...</Text>
          </View>
        ) : activeSchedules.length === 0 ? (
          <View className="home__state">
            <Text className="home__state-title">本周暂无待上课程</Text>
          </View>
        ) : (

...
func ListPublishedAnnouncementsForUser(db *gorm.DB, userID uint, page, pageSize int) ([]Announcement, map[uint]bool, int64, error) {
	...
	_ = db.Where("user_id = ? AND announcement_id IN ?", userID, ids).
		Find(&reads).Error
	for _, r := range reads {
...
WHERE a.deleted_at IS NULL AND a.status = ?
AND NOT EXISTS (
  SELECT 1 FROM announcement_reads r
  WHERE r.announcement_id = a.id AND r.user_id = ? AND r.deleted_at IS NULL
)
ORDER BY a.priority DESC, a.published_at DESC, a.id DESC
LIMIT ?
`, AnnouncementStatusPublished, userID, limit).
		Scan(&list).Error
	return list, err
}
...
func MarkAnnouncementRead(db *gorm.DB, announcementID, userID uint) error {
	...
	err := db.Where("announcement_id = ? AND user_id = ?",
		announcementID, userID).First(&existing).Error
	if err == nil {
		return nil
	}
...
func PublishAnnouncement(db *gorm.DB, id uint, operator string) error {
	now := time.Now()
	return db.Model(&Announcement{}).Where("id = ?", id).
